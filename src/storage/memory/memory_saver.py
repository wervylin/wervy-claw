    ...
    def _connect_with_retry(self, db_url: str) -> Optional[psycopg.Connection]:
        ...
        for attempt in range(1, DB_MAX_RETRIES + 1):
            try:
                ...
                # 检查是否是管理员终止连接的错误
                if "administrator command" in error_msg or "terminating connection" in error_msg:
                    logger.warning(f"Database connection terminated by administrator, will fallback to MemorySaver")
                    return None
